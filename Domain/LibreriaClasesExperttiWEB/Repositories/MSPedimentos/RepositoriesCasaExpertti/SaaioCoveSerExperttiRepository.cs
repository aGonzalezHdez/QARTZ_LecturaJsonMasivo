...
{
    public class SaaioCoveSerExperttiRepository : ISaaioCoveSerExperttiRepository
    {
        public string SConexion { get; set; }
        string ISaaioCoveSerExperttiRepository.SConexion { get => throw new NotImplementedException(); set => throw new NotImplementedException(); }

        public IConfiguration _configuration;
     

        public SaaioCoveSerExperttiRepository(IConfiguration configuration)
        {
        ...
        public List<SaaioCoveSer> CargarSeries(string NUM_REFE, int CONS_FACT, int CONS_PART)
        {
            ...
            try
            {
                ...
                using SqlCommand cmd = new("NET_SEARCH_SAAIO_COVESER_GRID_EXPERTTI", con)
                {
                ...
                while (dr.Read())
                {
                    var objSaaioCoveSer = new SaaioCoveSer
                    {
                        NUM_REFE = dr["NUM_REFE"].ToString(),
                        CONS_FACT = dr.GetInt32(dr.GetOrdinal("CONS_FACT")),
                        CONS_PART = dr.GetInt32(dr.GetOrdinal("CONS_PART")),
                        CONS_SERI = dr.GetInt32(dr.GetOrdinal("CONS_SERI")),
        ...
        public DataTable Cargar(string NUM_REFE)
        {
            ...
            {
                using SqlConnection con = new(SConexion);
                using SqlCommand cmd = new("NET_SEARCH_SAAIO_COVESER_NUM_REFE_EXPERTTI", con)
                {
                    CommandType = CommandType.StoredProcedure
                };
            ...
            {
                throw new Exception(ex.Message.ToString() + "NET_SEARCH_SAAIO_COVESER_NUM_REFE_EXPERTTI");
            }
        ...
        public int Insertar(SaaioCoveSer lsaaio_coveser)
        {
            ...
            {
                using SqlConnection con = new (SConexion);
                using SqlCommand cmd = new("NET_INSERT_SAAIO_COVESER_EXPERTTI", con)
                {
                    CommandType = CommandType.StoredProcedure
                };
        ...
        public int Modificar(SaaioCoveSer lsaaio_coveser)
        {
            ...
            {
                using SqlConnection con = new(SConexion);
                using SqlCommand cmd = new("NET_UPDATE_SAAIO_COVESER_EXPERTTI", con);
                {
            ...
            {                
                throw new Exception(ex.Message.ToString() + "NET_UPDATE_SAAIO_COVESER_EXPERTTI");
            }            
            return id;
        }
        ...
        public bool Borrar(string NUM_REFE, int CONS_FACT, int CONS_PART, int CONS_SERI)
        {
            ...
            {
                using SqlConnection con = new(SConexion);
                using SqlCommand cmd = new("NET_DELETE_SAAIO_COVESER_EXPERTTI", con)
                {
                    CommandType = CommandType.StoredProcedure             
                };   
            ...
            {                
                throw new Exception(ex.Message.ToString() + "NET_DELETE_SAAIO_COVESER_EXPERTTI");
            }           
            return id;
        }
        ...
        {            
            ...
            {
                using SqlConnection con = new(SConexion);
                using SqlCommand cmd = new("NET_SEARCH_SAAIO_COVESER_EXPERTTI", con)
                {
                    CommandType = CommandType.StoredProcedure
                };              
        ...
        public bool ActualizaFechaEntrada(string NumeroReferencia, DateTime FechaEntrada)
        {
            bool result = false;
            ...
            {
                using SqlConnection con = new (SConexion);
                using (SqlCommand cmd = new("NET_ACTUALIZA_FECHA_ENTRADA", con)) 
                {
                    con.Open();
                    cmd.CommandType = CommandType.StoredProcedure;

                    cmd.Parameters.Add("@NUM_REFE", SqlDbType.VarChar, 15).Value = NumeroReferencia;
                    cmd.Parameters.Add("@FEC_ENTR", SqlDbType.Date).Value = FechaEntrada;
            ...
            {                
                throw new Exception(ex.Message.ToString() + "NET_ACTUALIZA_FECHA_ENTRADA");
            }            
            return result;
        }
    }
